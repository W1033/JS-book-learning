...
/*
*  ES5 添加了 indexOf() 和 lastIndexOf() 两个方法，用于在数组中查找特定的值，但每次只能查找一个值。
*  如果想在一系列数字中查找一个偶数，则必须自己编写代码。于是 ES6 引入了 find() 和 findIndex() 方法
*  来解决这个问题。
*       - find() 方法返回查找到的值。
*       - findIndex() 方法返回查找到的值的索引。
*  这2个方法都接受2个参数: 1.一个回调函数; 2.可选参数，用于指定回调函数中 this 的值。 执行回调函数时，
*  传入的参数分别为: (1). 数组中的某个元素; (2).该元素在数组中的索引; (3).数组本身. 与传入 map() 和
