...
- 执行顺序解说:
    + (0) 代码执行 首先把全局执行上下文(global context) 推入到执行栈中(下图的 `Step1`),
      接下来是给构造函数 Function 的原型上添加 before(行{1}) 和 after(行{6}) 方法, 
      方法没有调用不会创建对应的函数执行上下文, 所以不会被推入到执行栈.
    + (1) 接下来我们看 行{12} 使用 函数表达式 定义一个函数 func. [tip: 此处相同表述为 --
      用函数表达式的方式声明一个函数并赋值给变量 func.]
        - Note01: 补充说明, 在《JavaScript高级程序设计》的 `5.5 Function 类型`
          一节中明确指出: `在 ECMAScript 中函数实际上是对象. 每个函数都是 Function`
          `类型的实例, 而且都与其他引用类型一样具有属性和方法.` 行{12}的写法使用 Function
    ...
    + (2) 行{14} 把 func.before() 再次赋值给 func, 此时 Function 构造函数原型上的
      before() 方法被调用, 立马创建此函数执行上下文, 并推入到执行栈中(下图 `Step2`),  
      (Note02: 我们知道把函数(/对象)赋值给变量, 变量引用的只是函数的指针, 我们再次把
       func.before() 赋值给 func 变量就相当于把 func 的引用从 A 改到 B.)  
      现在我们进入 before() 方法内, 看看其内部代码的执行,首先 行{2} 声明一个 `_self` 变量,
      用来保存 `this`(即: 行{12} Function构造函数的实例(即:`f2`)), 接着执行 return 语句,
      注意, 此处 return 返回的是一个闭包 (`闭包: 即有权访问另一个函数作用域中变量的函数`), 
      因为闭包的存在, func.before() 函数的执行上下文不会被释放, 所以此时不会从执行栈中弹出,
      在 return 返回的匿名函数(即:`r1`) 内部 行{3} 和 行{4} 引用的 this 都指向 window,
      原因是返回的函数(r1)被放在了全局执行上下文中; 这里要注意 beforefn(即:`f1`) 为执行
      before()方法传入的参数, 但在当前方法返回的闭包中被使用.
    + (3) 我们继续看 {16}行, Function.prototype 上的 after() 方法被调用, 
      立即创建当前函数的执行上下文, 并且 push 到执行栈中(下图的 `Step3`), 
      同样我们进入 after() 方法内部去看, 首先是 行{6}, 行{6} 和 行{2} 代码一样,
      但注意 _self 指向的 this 却是不同的, 这里的 this 指向是 Function.prototype
      上的 before() 方法执行完毕后返回的匿名函数(即:`r`) [在代码中 console 查看],
      然而 before() 方法内的 this 指向的是 行{12} (即:`f2`). 接下来 return
      照样返回一个匿名函数(即:`r3`)到全局执行上下文中, 由于 r3 存在对外围 after()
      函数的引用, 所以 after() 的函数执行上下文也不会被释放, 因此也不会从执行栈中弹出函数.
    + (4) 执行 func(), 创建此函数执行上下文后, 把 func() 推入到执行栈中(下图的 `Step4`),
      到这里是不是炸了锅? 嗯~~~??? func() 调用执行什么? 少年莫慌, 听老衲细细到来, 
      我们回头来看一下代码
